...
public class NetworkLayerServer {
       ...
       // simpleDVR(1);
        ...
        while(true) {
            try {
                ...
                new ServerThread(networkUtility, endDevice);
    ...
    public static synchronized void DVR(int startingRouterId) {
        ...
        int c = 0;
        while (c < routers.size()){
            //int updateCount = 0;
            for (int i = 0; i < routers.size(); i++){
                int id = (startingRouterId -1 + i)%routers.size();

                ArrayList<Integer> neighborlist = routers.get(id).getNeighborRouterIDs();
                for (int j = 0; j < neighborlist.size(); j++){
                    int neighbor = neighborlist.get(j);

                    if (routers.get(neighbor-1).getState()){
                        boolean bool = routers.get(neighbor-1).sfupdateRoutingTable(routers.get(id));
                        //if(bool)
                        //    updateCount++;
        ...
        while (true){
            ...
            for (int i = 0; i < routers.size(); i++){
                ...
                for (int j = 0; j < neighborlist.size(); j++){
                    ...
                    if (routers.get(neighbor-1).getState()){
                        boolean bool = routers.get(neighbor-1).updateRoutingTable(routers.get(id));
    ...
    public static EndDevice getClientDeviceSetup() {
        ...
        for (Map.Entry<IPAddress, Integer> entry : clientInterfaces.entrySet()) {
            ...
            if(i == r) {
                ...
                //System.out.println(endDevices.size() +" .. "+interfacetoRouterID.get(key));
                break;
            }
            i++;
        }
